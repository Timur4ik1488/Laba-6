def f(s):
    ...
    #обработка последнего слова в строке, если это не пробел
    if cur_word:
        if total_count>0:
            r=(count_a + count_b)/total_count
        else:
            r=0
        ...
        if r > max_r:
            ...
            res_words = [*res_words, cur_word]

    return res_words

s="Я вообще хз, каааак должен работать это код, но он работает, поэтому нужно следовааааать золотому правилу программиста: работаааает -  не трожь"
words_with_max_r = f(s)
print(words_with_max_r)
